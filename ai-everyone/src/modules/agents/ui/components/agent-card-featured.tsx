"use client";
// Featured agent card — larger variant for the hero/featured section.
...
interface AgentCardFeaturedProps {
  ...
  /** When true, renders a tall "hero" variant. */
  large?: boolean;
}
...
export const AgentCardFeatured = ({
  ...
  return (
    ...
    >
      ...
      {/* Deep gradient overlay anchoring text at bottom */}
      <div className="pointer-events-none absolute inset-0 z-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent" />

      {/* Top badges (e.g. Category / Game Pass style) */}
      <div className="absolute left-6 top-6 z-20 flex gap-2 w-[calc(100%-48px)] justify-between items-start">
      ...
      <div className="relative z-10 mt-auto flex flex-col items-start pt-32">
        ...
        {/* Short Description */}
        <p className={`mb-5 text-white/60 ${large ? "text-[15px] max-w-sm line-clamp-2" : "text-[13px] line-clamp-1"}`}>
          {agent.description}
        </p>
