...
const COMPOSER_VERSION = "composer-v4";
const DEBUG_BUILD_ID = process.env.NEXT_PUBLIC_DEBUG_BUILD_ID || "debug-build-missing";
const SHOW_DEBUG_BADGE = process.env.NODE_ENV !== "production";

export interface ChatComposerProps {
    onFirstMessage?: () => void;
}

export const ChatComposer: React.FC<ChatComposerProps> = ({ onFirstMessage }) => {
    const {
        sendMessage,
        isGenerating,
        isStopping,
    ...
    const isHomeSurface = activeChatId === null && messages.length === 0;
    const debugBadgeText = `${COMPOSER_VERSION} | ${DEBUG_BUILD_ID}`;

    return (
        <div
            className="w-full px-4 pb-6 pt-2"
            data-composer-version={COMPOSER_VERSION}
            data-debug-build-id={DEBUG_BUILD_ID}
        >
                ...
                >
                    {isVoiceActive ? (
                        ...
                        <div className="space-y-2">
                            {SHOW_DEBUG_BADGE && isHomeSurface && (
                                <div className="flex justify-end px-1">
                                    <span className="rounded-md border border-white/10 bg-white/5 px-2 py-0.5 font-mono text-[10px] tracking-tight text-cyan-300">
                                        {debugBadgeText}
                                    </span>
                                </div>
                            )}
