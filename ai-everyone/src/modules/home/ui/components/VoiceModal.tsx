...
import { MicrosoftLoginCard, type DeviceFlowData } from "@/modules/chat/ui/components/microsoft-login-card"
...
interface Props {
  ...
  authFlowData?: DeviceFlowData | null
  onAuthComplete?: () => void
}
...
// ── Main component ────────────────────────────────────────────────────────────
export default function VoiceModal({ onTranscript, onClose, authFlowData, onAuthComplete }: Props) {
  ...
  const startListening = () => {
    ...
    rec.onstart = () => {
      ...
      console.log("[VoiceModal] Started listening...");
    }
    ...
    rec.onresult = (e: any) => {
      ...
      // Always store the most aggressively recent text in case onend fires before isFinal
      interimTextRef.current = text;
      setLiveText(text);
      console.log("[VoiceModal] Interim result:", text);
    }

    rec.onerror = (e: any) => {
      console.warn("[VoiceModal] Speech recognition error:", e.error);
    ...
    rec.onend = () => {
      ...
      console.log("[VoiceModal] Session ended. Captured text:", said);
  ...
  return (
    <>
      ...
      >
        ...
        {/* Circle or Auth Card */}
        {authFlowData ? (
          <div style={{ position: "relative", marginBottom: 20, flexShrink: 0, display: "flex", justifyContent: "center" }}>
            <MicrosoftLoginCard
              deviceData={authFlowData}
              onAuthenticated={() => {
